<template>
	<div>
		...
		<Product 
			v-for="(item, index) in filteredAndOrderedList" 
			:key="index"
			:info="item"></Product>
		<div class="product-not-found" v-show="!filteredAndOrderedList.length">暂无商品</div>
	</div>
</template>
		...
		methods: {
			...
			handleOrderCost() {
				if(this.order === 'cost-asc'){
					this.order = 'const-desc'
				}else {
					this.order = 'const-asc'
				}
			},
		},
		computed: {
